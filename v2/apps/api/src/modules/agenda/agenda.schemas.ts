...
export const createAgendaEventSchema = z.object({ title: z.string().min(1).max(255), taskDescription: z.string().max(4000).nullable().optional(), startsAt: z.string().min(1), endsAt: z.string().nullable().optional(), timeZone: z.string().min(1).max(64).optional(), color: z.string().regex(/^#[0-9a-fA-F]{6}$/).default("#c9f7df"), clientAccountId: z.string().uuid().nullable().optional(), labelId: z.string().uuid().nullable().optional(), recurrenceType: recurrenceType.default("none"), repeatUntil: z.string().nullable().optional(), meetLink: z.string().max(500).nullable().optional() });
export const updateAgendaEventSchema = createAgendaEventSchema.partial().extend({ title: z.string().min(1).max(255).optional(), isCompleted: z.boolean().optional() });
