...
function InvoiceEditor({ invoice, clients, session, onBack, onChange, onDelete }: { invoice: BillingInvoice; clients: Array<{ id: string; name: string; slug: string }>; session: SessionUser; onBack: () => void; onChange: (invoice: BillingInvoice) => void; onDelete: () => void }) {
  ...
  const text = labels[invoice.locale];
  const printInvoice = () => {
    const paper = invoiceCanvasRef.current?.querySelector(".invoice-paper");
    if (!paper) return;
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8" /><title>${text.invoice} #${invoice.number}</title>${headMarkup}<style>html,body{margin:0;background:#fbfaf8;}body{display:flex;justify-content:center;padding:24px;}.invoice-paper{box-sizing:border-box;width:100%;max-width:720px;min-height:0;margin:0;box-shadow:none;}@media print{@page{size:A4;margin:14mm;}html,body{width:auto;min-height:0;margin:0;padding:0;background:#fff;}body{display:block;}.invoice-paper{width:100%;max-width:none;min-height:0;margin:0;padding:34px 38px 28px;box-shadow:none;break-inside:avoid-page;}}</style></head><body>${paper.outerHTML}</body></html>`);
  ...
  return <section className="invoice-editor">
    ...
    <main className="invoice-canvas" ref={invoiceCanvasRef}><div className="invoice-canvas-actions"><button onClick={() => update("status", invoice.status === "paid" ? "open" : "paid")}>{invoice.status === "paid" ? "Marcar como aberta" : "Marcar como paga"}</button><button onClick={printInvoice}>Baixar PDF</button><button className="send" onClick={() => { setSent(true); update("visibleToClient", true); }}>{sent ? "Enviada para o cliente" : "Enviar ao cliente"}</button></div><BillingInvoiceDocument invoice={invoice} /></main>
