...
function seedInvoices(): BillingInvoice[] {
  return [
    {
      id: "invoice-56", number: 56, title: "Gestão de redes sociais", clientName: "Aplikasi", clientEmail: "contato@aplikasi.com", clientAddress: "Rua do Comércio, 120", clientCountry: "Brasil", clientTaxId: "CNPJ 00.000.000/0001-00", issueDate: "2026-08-01", dueDate: "2026-09-09", period: "01/08/2026 - 31/08/2026", currency: "BRL", locale: "pt", status: "open", recurring: true, fixedAmount: true, visibleToClient: true, attachmentName: "nota-fiscal-56.pdf", notes: "Pagamento via transferência bancária.", lines: [{ id: "line-56", description: "Gestão de redes sociais - Agosto 2026", quantity: 1, unitPrice: 1776 }],
    },
    {
      id: "invoice-55", number: 55, title: "Consultoria mensal", clientName: "Elite Leader Podcast", clientEmail: "hello@eliteleader.com", clientAddress: "Av. Paulista, 400", clientCountry: "Brasil", clientTaxId: "", issueDate: "2026-08-01", dueDate: "2026-09-09", period: "01/08/2026 - 31/08/2026", currency: "BRL", locale: "pt", status: "open", recurring: true, fixedAmount: false, visibleToClient: true, attachmentName: "", notes: "", lines: [{ id: "line-55", description: "Consultoria estratégica", quantity: 1, unitPrice: 950 }],
    },
    {
...
function emptyInvoice(nextNumber: number): BillingInvoice {
  ...
  return { id: crypto.randomUUID(), number: nextNumber, title: "Nova fatura", clientName: "", clientEmail: "", clientAddress: "", clientCountry: "", clientTaxId: "", issueDate: today, dueDate: today, period: "", currency: "BRL", locale: "pt", status: "open", recurring: false, fixedAmount: true, visibleToClient: false, attachmentName: "", notes: "", lines: [{ id: crypto.randomUUID(), description: "Serviço", quantity: 1, unitPrice: 0 }] };
}
...
export function BillingWorkspace({ session, newInvoiceSignal = 0 }: { session: SessionUser; newInvoiceSignal?: number }) {
  ...
  useEffect(() => {
    listAdminClients()
      .then(({ items }) => { setClients(items); setLogosByClient(Object.fromEntries(items.filter((client) => client.logo_url).map((client) => [client.name.trim().toLocaleLowerCase(), client.logo_url as string]))); })
