...
function DashboardNotesWidget({ userId, canPersist }: { userId: string; canPersist: boolean }) {
  ...
  return <>
    {notes.length ? <section className="dashboard-postits-board dashboard-first-row-widget" aria-label="Meus lembretes"><header><div><span className="dashboard-notes-icon">✦</span><h3>Meus lembretes</h3></div><button type="button" onClick={() => window.dispatchEvent(new Event("design-hub:open-dashboard-note"))} aria-label="Criar nova nota" title="Nova nota">＋</button></header><div className="dashboard-notes-list">{notes.slice(0, 2).map(renderNote)}</div>{notes.length > 2 ? <button className="dashboard-notes-more" type="button" onClick={() => setAllNotesOpen(true)}>+ {notes.length - 2} {notes.length - 2 === 1 ? "lembrete" : "lembretes"}</button> : null}</section> : null}
