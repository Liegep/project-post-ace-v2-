...
function AdminTextsView({ clientName, slug, onCountChange }: { clientName: string; slug: string; onCountChange?: (count: number) => void }) {
  ...
  const addTextTag = () => { const tag = tagDraft.trim().replace(/^#/, ""); if (!selected || !tag || (selected.tags ?? []).some((item) => item.toLocaleLowerCase("pt-BR") === tag.toLocaleLowerCase("pt-BR"))) return; updateLocal({ tags: [...(selected.tags ?? []), tag].slice(0, 12) }); setTagDraft(""); };
  const removeTextTag = (tag: string) => updateLocal({ tags: (selected?.tags ?? []).filter((item) => item !== tag) });
  ...
  return <section className="texts-studio" ref={studioRef} onScroll={(event) => { const next = event.currentTarget.scrollTop > 180; setShowScrollTop((current) => current === next ? current : next); }}>
    ...
    <article className="texts-document">
      ...
      <div className="texts-paper">
        ...
        <header className="texts-paper-head"><div><span className="texts-status">{selected.status}</span>{selected.tags?.length ? <div className="text-heading-tags">{selected.tags.map((tag) => <span key={tag}>{tag}</span>)}</div> : null}<textarea className="texts-title-input" rows={1} value={selected.title} onChange={(event) => updateLocal({ title: event.target.value.replace(/\n/g, " ") })} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); event.currentTarget.blur(); } }} aria-label="Título do texto" /><div className="texts-meta"><span>◇ {selected.contentType}</span><span>◷ Atualizado {new Intl.DateTimeFormat("pt-BR", { dateStyle: "medium" }).format(new Date(selected.updatedAt))}</span></div></div></header>
    ...
    <aside className="texts-side-panel">
      ...
      <section className="texts-tags-panel"><span>Etiquetas</span><div className="texts-tags-list">{(selected.tags ?? []).map((tag) => <button key={tag} onClick={() => removeTextTag(tag)} title={`Remover ${tag}`}>{tag}<b>×</b></button>)}</div><div className="texts-tag-create"><input value={tagDraft} maxLength={40} onChange={(event) => setTagDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addTextTag(); } }} placeholder="Nova etiqueta" /><button onClick={addTextTag} disabled={!tagDraft.trim()}>＋</button></div></section>
