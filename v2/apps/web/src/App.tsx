          ...
          {mobileAgendaDays.map((day) => {
            ...
            return <article className={`social-agenda-day${isToday ? " today" : ""}`} key={key}>
              ...
              <div className="social-agenda-items">
                {dayEvents.map((item) => { const visual = agendaEventVisualState(item); return <button key={item.id} type="button" className={`social-agenda-item appointment${visual.className}`} style={{ "--calendar-event-color": visual.color } as CSSProperties} onClick={() => { setSelectedEvent(item); setRescheduleAt(toDateTimeLocal(item.startsAt)); setMeetLinkEdit(item.meetLink ?? ""); setClientAccountIdEdit(item.clientAccountId ?? ""); }}><span className="social-agenda-time">{formatAgendaTime(item.startsAt)}</span><i><UiIcon name="clock" /></i><span><strong>{item.title}</strong><small>{item.taskDescription || item.labelName || item.clientName || "Compromisso"}</small></span><b>›</b></button>; })}
